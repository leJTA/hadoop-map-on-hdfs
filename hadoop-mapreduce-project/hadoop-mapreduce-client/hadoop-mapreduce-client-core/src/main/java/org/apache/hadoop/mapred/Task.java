   ...
   private long calculateOutputSize() throws IOException {
    ...
    if (isMapTask() && conf.getNumReduceTasks() > 0) {
      try {
        ...
        FileSystem defaultFS = FileSystem.get(conf); // JTA
        LOG.warn("Output size of " + mapOutput.toString() + " is " + defaultFS.getFileStatus(mapOutput).getLen());
        return defaultFS.getFileStatus(mapOutput).getLen();
      } catch (IOException e) {
